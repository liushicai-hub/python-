...
# b表示通过ascii码表二进制编码 \n
def ShowRun(DeviceName, MgmtIP, Username, Password):
...
def ChangePass(DeviceName, MgmtIP, Username, Password, NewPass):
    tn = telnetlib.Telnet(MgmtIP)
    ...
    tn.read_until('{}#'.format(DeviceName).encode('ascii'))
    tn.close()
    NewInfo = DeviceName + ',' + MgmtIP + ',' + Username + ',' +Password +','+ NewPass + '\n'
    return NewInfo
...
def config_bkp(DeviceName, Config):
    File = open(DeviceName + '_' +DateTime + '.txt', mode='w')
    File.write(Config)
    File.close()
...
def SaveNewInfo(DateTime,NewInfo):
    NewLoginFile = 'newlogin{}.txt'.format(DateTime)
...
if __name__ == ('__main__'):
    ...
    if Choice == '1':
        for i in LoginInfo:
            #实例化登录信息变量
            DeviceName = i[0]; Ip = i[1]; Username = i[2]; Password = i[3]
            #实例化获取到的配置
            config = ShowRun(DeviceName, Ip, Username, Password)
            #将配置写入文件
        ...
        for i in LoginInfo:
            ...
            NewPass = generateKey()
            #修改密码并返回新的登录信息
            NewInfo = ChangePass(DeviceName, Ip, Username, Password, NewPass)
